  ...
  return (
    <div className="app">
      <aside className="panel">
        ...
        <p className="hint">{t.single_hint}</p>

        <div className="lang-row">
          <label className="lang-label">{t.lang_label}</label>
        ...
        <div className="toolbar">
          <button className="btn" onClick={() => setCode("")}>{t.clear}</button>
          <button
            className="btn"
            onClick={() => {
        ...
        <div className="stats">
          <div className="stat-row"><span>{t.nodes}</span><b>{result.nodes.length}</b></div>
          <div className="stat-row"><span>{t.calls}</span><b>{callEdges}</b></div>
          <div className="stat-row"><span>{t.dataflow}</span><b>{usesEdges}</b></div>
        </div>

        <div className="legend">
          {activeKinds.length === 0 && <div className="legend-empty">{t.no_nodes}</div>}
          {activeKinds.map((k) => (
        ...
        <footer className="foot">
          ...
          >
            <img src={`${import.meta.env.BASE_URL}assets/buy-me-a-coffee.png`} alt="Buy Me a Coffee" />
          </a>
          <span>{t.single_foot} · {LANGUAGES.length - 1} {t.languages}</span>
        </footer>
      </aside>
      ...
      <main className="canvas">
        ...
        {result.nodes.length === 0 && !result.error ? (
          <div className="empty">
            ...
            {debounced.trim() ? t.nothing_detected : t.no_code}
          </div>
        ) : (
          <ReactFlow
            nodes={nodes}
